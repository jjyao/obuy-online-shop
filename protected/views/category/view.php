<?php $this->pageTitle=Yii::app()->name . ' -' . $category->name; 
Yii::app()->clientScript->registerLinkTag('stylesheet/less', 'text/css', Yii::app()->baseUrl.'/css/index.less');
?>
<article class="container">
<nav class="breadcrumbs">
<?php
	// get all ancestor categories and current category
	$categories = array();
	$categories[] = $category;
	$tempCategory = $category;
	if($tempCategory->parentCategory != null)
	{
		$tempCategory = $tempCategory->parentCategory;
		array_unshift($categories, $tempCategory);
	}
?>
<ul>
	<li class="first">
	...
	<ul id="category_menu">
		<?php
			...
			else
			{
				...
				{
					echo $subCategory->getMenuRepresent();
				}
			}
		?>
	</ul>
